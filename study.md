...
```md
Both require callbacks which take in things like element and index as an arguement.
Ruby has many more built-in methods that save us from having to write extra
functions.
```
...
```md
The Ruby method is creating a class while JS is creating an object.

The Ruby method can loop through all of the rows in the file using a foreach
loop while JS uses a while loop
```
...
```md
Couldn't quite figure out how to get this function to work. 
```
